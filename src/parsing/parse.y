...
|       cmds EOC_TOK {
  push_back_Cmds(&$1, make_command_holder(NULL, NULL, 0, make_eoc()));
...
cmd_top: cmd_content redir cmd_bg {
  ...
  $$ = make_command_holder($2.in, $2.out, flags, $1);
}



cmd_content: cmd {
  $$ = make_generic_command(as_array_CmdStrs(&$1, NULL));
}
|       ECHO_TOK {
  char** cmd = memory_pool_alloc(sizeof(char*));
  *cmd = NULL;
  $$ = make_echo_command(cmd);
}
|       ECHO_TOK cmd_arguments {
  $$ = make_echo_command(as_array_CmdStrs(&$2, NULL));
}
|       EXPORT_TOK ID EQUALS string {
  $$ = make_export_command($2, $4);
}
|       CD_TOK {
  $$ = make_cd_command(memory_pool_strdup(lookup_env("HOME")));
}
|       CD_TOK string {
  char* resolved_path;
  char* ret = NULL;
...
|       redir_mark string {
  ...
  if ($1 == REDIRECT_IN)
    r = make_redirect($2, NULL, false);
  else if ($1 == REDIRECT_OUT)
    r = make_redirect(NULL, $2, false);
  else if ($1 == REDIRECT_APPEND)
    r = make_redirect(NULL, $2, true);
  else
