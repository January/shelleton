 ...
 * @sa Redirect
 */
Redirect make_redirect(char* in, char* out, bool append);
 ...
 * @a Command structure to @a ShelletonState
 *
 * @param[out] state The state of the shelleton shell. The parsed_str member of
 * ShelletonState is set to the stringified command structure.
 *
 * @return A pointer to the parsed command structure
 *
 * @sa CommandHolder, ShelletonState
 */
CommandHolder* parse(ShelletonState* state);
